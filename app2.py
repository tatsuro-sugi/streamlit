...
df = pre_df[:-1]
df_2=df.drop_duplicates('姓')

#小テストの受講状況「状態」をdf_test_stateに代入
df_test_state = df_2[['状態']]
...
st.markdown(f'##### 受験率は{test_rate2}です。')

test_rate3=float(test_rate)
if test_rate3 == 1.0:
    st.markdown(f'##### 全員受験してますね、素晴らしいです。')
elif test_rate3 > 0.5:
    st.markdown(f'##### 半分以上の学生が受験していますね、良い感じです。')
else:
    st.markdown(f'##### まずは受験しましょう。')
...
if average_score == 10:
    st.markdown(f'##### 頑張ってますね')
elif average_score > 6:
    st.markdown(f'##### もう一息です。')
else:
    st.markdown(f'##### 全体的に苦戦しているようです。わからないことはそのままにせず、教員に相談しましょう。')
...
st.markdown(f'##### 最高点は{max_score}点、最低点は{min_score}点でした。')
...
with col1:
   #---受験状況をグラフ化---
    ...
    ax.axis("equal")
    st.pyplot(fig)
    st.caption('###### 受験状況・・・🟧受験済み　🟦未受験')
    
#col2に小テストの得点分布をhistで表示
with col2:
  #---ヒストグラムを表示---    arr = np.random.normal(1, 1, size=100)
    fig, ax = plt.subplots()
